...
import { paymentCreatePaymentSchema, paymentSchema, } from "../../utils/schema.js";
const JWT_SECRET = process.env.JWT_SECRET || "secret";
export const paymentMiddleware = async (c, next) => {
...
export const paymentCreatePaymentMiddleware = async (c, next) => {
    const userData = c.get("user");
    // if (
    //   userData.user_metadata.role !== "MEMBER" &&
    //   userData.user_metadata.role !== "ADMIN" &&
    ...
    //   return c.json({ message: "Unauthorized" }, 401);
    // }
    const { order_number, payment_method, payment_details, payment_type } = await c.req.json();
...
export const paymentGetMiddleware = async (c, next) => {
    const userData = c.get("user");
    // if (
    //   userData.user_metadata.role !== "MEMBER" &&
    //   userData.user_metadata.role !== "ADMIN" &&
    ...
    const isAllowed = await rateLimit(`rate-limit:${userData.id}:payment-get`, 50, "1m", c);
    if (!isAllowed) {
        return c.json({ message: "Too many requests" }, 429);
    }
