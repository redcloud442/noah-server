...
export const webhookMiddleware = async (c, next) => {
    ...
    const sigParts = Object.fromEntries(signatureHeader.split(",").map((p) => p.trim().split("=")));
    ...
    const isValid = suppliedSignature.length === computed.length && // lengths must match
        timingSafeEqual(Buffer.from(computed, "hex"), Buffer.from(suppliedSignature, "hex"));
    if (!isValid)
