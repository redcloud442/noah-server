...
import { orderGetListSchema, orderPutSchema } from "../../schema/schema.js";
...
export const orderPutMiddleware = async (c: Context, next: Next) => {
  const user = c.get("user");
  ...
  const validated = orderPutSchema.safeParse(formattedData);
